...
class TransformerBlock(nn.Module):
    ...
    def forward(
        ...
        # h = h.clamp(min=-65500, max=65500)
        # print_nan_info(h, f"attn:layer {i}")
        out = h + self.feed_forward.forward(self.ffn_norm(h))
        # out = out.clamp(min=-65500, max=65500)
        # print_nan_info(out, f"ffn:layer {i}")
        return out
...
def print_nan_info(tensor, tensor_name):
    if torch.isnan(tensor).any():
        print(f"NaN detected in {tensor_name}")
    else:
        print(f"{tensor.size()} detected in {tensor_name}")
