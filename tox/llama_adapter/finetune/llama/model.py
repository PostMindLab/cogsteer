        ...
        if adapter is not None:
            scores = torch.cat(
                [
                    ...
                    self.gate.tanh().float() * F.softmax(scores[:, :, :, :adapter_len].float(), dim=-1).type_as(xq),
...
class Transformer(nn.Module):
    ...
    def forward(self, examples, labels):
        ...
        with torch.no_grad():
            ...
            start_pos = 0
            if self.adapter_layer != 32: # 32 is the full model, not freezed prior layers
                for layer in self.layers[: self.adapter_layer]: 
                    h = layer(h, start_pos, freqs_cis, mask)

        adapter_index = 0
        adapter = self.adapter_query.weight.reshape(-1, self.adapter_len, 4096).unsqueeze(1)
        if self.adapter_layer == 32:
            for layer in self.layers[-1 * self.adapter_layer :]:
                # h = layer(h, start_pos, freqs_cis, mask, adapter[adapter_index].half())
                h = layer(h, start_pos, freqs_cis, mask, adapter[adapter_index].float())
                adapter_index = adapter_index + 1

        else:
            ad_layer = self.layers[self.adapter_layer]
            h = ad_layer(h, start_pos, freqs_cis, mask, adapter[adapter_index].float())

            for layer in self.layers[self.adapter_layer+1:]:
                h = layer(h, start_pos, freqs_cis, mask)
