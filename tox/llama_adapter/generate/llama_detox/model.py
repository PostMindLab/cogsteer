...
class Attention(nn.Module):
    def __init__(self, args: ModelArgs):
        ...
        self.before_mergehead_last = nn.Identity()
    ...
    def forward(
        ...
        # output = self.before_mergehead(output)
        ...
        # output = self.before_mergehead_last(output)
        output = self.before_mergehead(output)
...
class Transformer(nn.Module):
    ...
    def forward(self, tokens: torch.Tensor, start_pos: int, adap_layer = None):
        ...
        elif adap_layer == "origin":
            # print("forward origin")
            for layer in self.layers:
                h = layer(h, start_pos, freqs_cis, mask)
        else:
            for layer in self.layers[: adap_layer]:
                h = layer(h, start_pos, freqs_cis, mask)

            ad_layer = self.layers[adap_layer]
            h = ad_layer(h, start_pos, freqs_cis, mask, prompt[0])

            for layer in self.layers[adap_layer+1:]:
                h = layer(h, start_pos, freqs_cis, mask)
