...
from transformers import TrainingArguments, LlamaForSequenceClassification, AutoTokenizer
...
def train(i):
    ...
    model.add_adapter(f"pt_layer{i}_{task}", config=config)
    model.train_adapter(f"pt_layer{i}_{task}")


    metric_name = "pearson" if task == "stsb" else "matthews_correlation" if task == "cola" else "accuracy"
    training_args = TrainingArguments(
        output_dir=f"./llama/temp/layer{i}_{task}", 
        do_train=True,
        learning_rate=1e-5,
        num_train_epochs=5, 
    ...
    val_result.append({"task": task, "layer": i, "type": "lora", "result": results})

    print('saving {}...'.format(i))
    model.save_adapter(f"llama/weights_pt/pt_layer{i}_{task}", f"pt_layer{i}_{task}")
...
for task in task_to_keys:
    ...
    json.dump(val_result, open(f"llama/val/{task}.json", "w"))
