import torch
import os
from transformers import GPT2Tokenizer, GPT2Model, GPT2ForSequenceClassification, AutoConfig
...
def eval(i):
    ...
    adapters.init(model)
    model.load_adapter(f"gpt2-large/weights_pt/pt_layer{i}_{task}")
    ...
    if is_test:
        ...
        df.index.name = "index"
        df.to_csv(output_path+f"{task.upper()}-mm.tsv", sep="\t", index=True)
    else:
...
for task in task_to_keys:
    ...
    for i in layers:
        ...
        if is_test:
            output_path = f"./gpt2-large/test/layer_{i}/" # for test
            Path(output_path).mkdir(parents=True, exist_ok=True)
        eval(i)
    if not is_test:
        json.dump(val_result, open(f"./gpt2-large/val/{task}.json", "w"), indent=2)
print(task_length)