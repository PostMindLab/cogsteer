import torch
import os
from transformers import MistralForSequenceClassification, AutoTokenizer, PretrainedConfig, AutoConfig
...
def eval(i):
    ...
    adapters.init(model)
    model.load_adapter(f"mistral/weights_pt/pt_layer{i}_{task}")
    ...
    if is_test:
        ...
        df.index.name = "index"
        df.to_csv(output_path+f"{task.upper()}.tsv", sep="\t", index=True)
    else:
...
for task in task_to_keys:
    ...
    for i in layers:
        ...
        if is_test:
            output_path = f"./mistral/test/layer_{i}/"
            Path(output_path).mkdir(parents=True, exist_ok=True)
        eval(i)
    if not is_test:
        json.dump(val_result, open(f"./mistral/val/{task}.json", "w"), indent=2)
